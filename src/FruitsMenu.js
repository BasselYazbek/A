...
const FruitsMenu = () => {
  ...
  return (
    <div className="container">
      ...
      />
      {Object.keys(categorizedItems).map((category, idx) =>
        categorizedItems[category].filter(item => item.name.toLowerCase().includes(searchTerm)).length > 0 ? (
          <section className="fruits-section" key={idx}>
            <h3 className="section-title">{getCategoryTitle(category)}</h3>
            <div className="row fruits-container">
              {categorizedItems[category].filter(item => item.name.toLowerCase().includes(searchTerm)).map((item) => (
                <Fruits key={item.id} {...item} />
              ))}
            </div>
          </section>
